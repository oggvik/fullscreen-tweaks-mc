...
At startup, the loading-screen setting can preserve Minecraft's regular fullscreen choice or temporarily force the native window to be windowed or fullscreen. GLFW targets through 26.2 create the window in windowed mode first, then apply the requested loading mode immediately after construction. The controller captures the decorated window rectangle before each fullscreen transition and explicitly reapplies it after leaving fullscreen, avoiding stale monitor-sized bounds on Windows. When a minimized windowed loading state must return to native fullscreen, it restores the window before attaching it to the monitor so Windows does not retain a hidden fullscreen presentation state. SDL3 targets select the loading mode directly during native-window creation. The controller records Minecraft's fullscreen option as well as the launch-time display value because modern Minecraft synchronizes those values later in its constructor. It reapplies the temporary mode after that synchronization and after Minecraft shows the window, which covers native-window handoff from NeoForge. When Minecraft clears its startup overlay, the controller restores the regular mode and restores the window from its minimized state.

Startup minimization occurs only after the selected GLFW fullscreen policy has been applied. This is important on Windows because iconifying a native-fullscreen window and subsequently detaching it into borderless mode can leave the detached window unmapped. On GLFW-based 26.2, restoration additionally shows the native window and reapplies the active fullscreen policy.
...
- Forge 1.16.5 uses the equivalent `ScreenSize` constructor argument on `net.minecraft.client.MainWindow`, but defers the loading fullscreen transition until `Minecraft` construction completes so framebuffer callbacks cannot observe partially initialized renderer state. For Windows sessions launched minimized with regular fullscreen enabled, the port preserves the constructor-time windowed rectangle across Forge's early-window handoff. It restores that rectangle before Minecraft's own `updateFullscreen()` transition, then restores, shows, and focuses the detached window without issuing a second monitor transition.
...
The settings button is attached to the main Options screen so replacements for video settings, including Sodium and Embeddium, do not remove the entry point. Its geometry helper checks visible widget bounds, tries aligned free slots, uses a compact button when necessary, and hides the control if no safe position exists. BTA 8.0.1 instead inserts its native controls into the Video page's Display category immediately after BTA's Fullscreen and Borderless Fullscreen settings.

Fabric settings builds bundle only the matching Fabric API base and resource-loader modules. Forge and NeoForge use their built-in resource-pack support.

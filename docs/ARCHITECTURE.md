...
On GLFW-based 26.2, restoring a minimized borderless window on Windows also explicitly shows the native window and reapplies the active fullscreen policy. This handles GLFW reporting the window as restored while Windows leaves the undecorated window unmapped until another mode transition.
